...
exports.organization = async (req, res) => {
  ...
  const {org_type,org_add}=req.body;
  ...
  try {
    ...
    const [orgUserData] = await findOrgUserByUserId(user_id,org_add,org_type);
    ...
    const [orgResult] = await insertOrganization(org_name,org_add,org_type);
